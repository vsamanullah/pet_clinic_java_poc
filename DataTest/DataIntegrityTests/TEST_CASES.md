...
### TC-PF-001: Snapshot Creation Performance

**Objective**: Verify snapshot creation completes within acceptable time

**Test Scenarios**:

| Data Size | Record Count | Expected Duration |
|-----------|--------------|-------------------|
| Baseline | 47 total records | < 3 seconds |
| Small | +20 additional (owners/pets) | < 5 seconds |
| Medium | +50 additional | < 8 seconds |
| Large | +100 additional | < 15 seconds |

**Test Command**:
```bash
time python create_snapshot.py --env local
```
...
| Data Size | Record Count | Expected Duration |
|-----------|--------------|-------------------|
| Baseline | 47 records | < 5 seconds |
| Small | +20 additional | < 10 seconds |
| Medium | +50 additional | < 20 seconds |
| Large | +100 additional | < 30 seconds |
...
### TC-NEG-002: Missing Snapshot File

**Objective**: Handle missing snapshot gracefully

**Test Command**:
```bash
python populate_test_data.py --snapshot nonexistent.json
```

**Expected Results**:
```
ERROR: Snapshot file not found: nonexistent.json
Please create snapshot first using create_snapshot.py
```

**Pass Criteria**:
- Helpful error message
- Suggests corrective action
- Script exits without crashing
...
### TC-NEG-003: Corrupted Snapshot File

**Objective**: Handle corrupted JSON snapshot
...
Please recreate snapshot using create_snapshot.py
```
...
**Test Steps**:
1. Create snapshot
2. Drop a table (e.g., DROP TABLE petclinic.types)
3. Run verification

**Expected Results**:
```
✗ types: Table not found in current database
  Expected table 'types' is missing from petclinic schema
...
**Test Steps**:
1. Create snapshot
2. Add new column to table (ALTER TABLE pets ADD COLUMN weight integer)
3. Run verification

**Expected Results**:
```
⚠ pets: Schema mismatch detected
  Current database has 6 columns, snapshot has 5 columns
  New column found: 'weight'
  Status: WARNING
```
...
**Test Steps**:
1. Populate data with NULL optional fields (address, city, telephone in owners)
2. Create snapshot
3. Run verification
...
```
✓ owners: Data checksum matched
  (NULL values handled correctly in checksum calculation)
```
...
**Objective**: Test special character handling in PostgreSQL

**Test Data**:
- Owner names with: O'Brien, José García, Müller
- Pet names with: "Max", <Buddy>, 'Charlie'
...
### TC-EDGE-004: Date Boundary Values

**Objective**: Test date field edge cases

**Test Data**:
- birth_date: Past dates (100 years ago)
- visit_date: Current date, past dates
- Date ranges spanning years

**Expected Results**:
- All dates stored correctly
- No date truncation or timezone issues
- PostgreSQL date type handles range properly
...
### TC-EDGE-005: Sequence Counter Reset

**Objective**: Test auto-increment sequence reset after snapshot load

**Test Steps**:
1. Load snapshot with max id = 10
2. Verify sequence is set to 10
3. Insert new record
4. Verify id = 11 (no duplicate key error)

**Expected Results**:
```
✓ Sequence reset successfully
  owners_id_seq current value: 10
  Next insert gets id: 11
...
|----------|-------------|----------|------|--------|-----|
| Schema Validation | 7 | 6 | 1 | 0 | 0 |
| Data Integrity | 13 | 7 | 6 | 2 | 0 |
| Migration Verification | 5 | 3 | 2 | 0 | 0 |
| Referential Integrity | 6 | 4 | 2 | 0 | 0 |
| Test Data Population | 5 | 2 | 3 | 0 | 0 |
| Performance | 4 | 0 | 0 | 3 | 1 |
| Negative Tests | 5 | 0 | 2 | 2 | 1 |
| Edge Cases | 5 | 0 | 1 | 2 | 2 |
| **TOTAL** | **50** | **22** | **17** | **9** | **4** |
...
|--------|-------|------------|
| Fully Automated | 43 | 86.0% |
| Partially Automated | 5 | 10.0% |
| Manual Only | 2 | 4.0% |
...
```bash
# Quick snapshot + baseline load test
python create_snapshot.py --env source
python populate_test_data.py --env local
python verify_migration.py --env local --snapshot ../petclinic_snapshot_source_*.json
```
...
python create_snapshot.py --env source --output baselines/regression_baseline.json
python populate_test_data.py --env source --additional 100
# Perform migration (external process)
python verify_migration.py --env target --snapshot baselines/regression_baseline.json
```
